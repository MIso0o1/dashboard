  ...
  const getCardHeight = () => {
    switch (widget.type) {
      case "todo":
        return "h-80"
      case "metric":
        return "h-64" // Increased height for better metric display
      case "lifestyle":
        return "h-80"
      default:
        return "h-80"
  ...
  const getCardStyle = () => {
    switch (widget.type) {
      case "finance":
        return "finance-card"
      case "health":
        return "health-card" // Orange for health widgets
      case "todo":
        return "productivity-card" // Purple for todo widgets
      case "investment":
        return "investment-card"
      case "lifestyle":
        return "lifestyle-card" // Green for lifestyle widgets
      case "metric":
        return "enhanced-card" // Clean look for metrics
      default:
    ...
    <>
      <Card className={`${getCardHeight()} flex flex-col hover-lift ${getCardStyle()}`}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 flex-shrink-0 glass-effect rounded-t-xl">
          <CardTitle className="text-sm font-bold truncate text-slate-800">{widget.title}</CardTitle>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 flex-shrink-0 hover:bg-white/50 focus-glow">
                <MoreHorizontal className="h-4 w-4 text-slate-600" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="glass-effect border-white/20 shadow-xl">
              <DropdownMenuItem onClick={() => setShowEditDialog(true)} className="hover:bg-white/50 text-slate-700">
                <Edit className="h-4 w-4 mr-2 text-blue-600" />
                Edit
              </DropdownMenuItem>
              <DropdownMenuItem onClick={onDelete} className="text-red-600 hover:bg-red-50/50">
        ...
        <CardContent className="flex-1 overflow-hidden p-2 bg-white/30">{renderWidgetContent()}</CardContent>
      </Card>
